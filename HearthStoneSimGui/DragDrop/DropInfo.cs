	...
	public class DropInfo : IDropInfo
	{
        ...
        public DropInfo(object sender, DragEventArgs e, DragInfo dragInfo)
		{
			...
			if (this.VisualTarget is ItemsControl)
			{
				...
				//System.Diagnostics.Debug.WriteLine($">>> Name = {itemsControl.Name}");
				...
				if (item != null)
				{
					...
					this.TargetItemIndex = itemParent.ItemContainerGenerator.IndexFromContainer(item);
					this.InsertIndex = this.TargetItemIndex;
					...
					else
					{
						...
						{
						    //System.Diagnostics.Debug.WriteLine($"==> DropInfoStart: {this.TargetItem} idx={InsertIndex}," +
						    //                                   $" prev_idx={DragDrop.PreviewInsertIndex}, X={currentXPos}");
                            if (DragDrop.PreviewInsertIndex == -1)  //preview insert not active
                            {
						        if (currentXPos > targetWidth * 0.5)
						        {
						            InsertIndex++;
                            ...
                            else if (DragDrop.PreviewInsertIndex != InsertIndex)
                            {
                                if (currentXPos > targetWidth * 0.2 && currentXPos < targetWidth * 0.8)
                                {
                                    if (currentXPos < targetWidth * 0.5)
                                    {
                                        InsertIndex++;
						...
						//System.Diagnostics.Debug.WriteLine($"==> DropInfo: pos={InsertPosition}, {this.TargetItem} idx={InsertIndex}," +
						//                                   $" prev_idx={DragDrop.PreviewInsertIndex}, X={currentXPos}");
					}
				}
				else
				{
					...
					//System.Diagnostics.Debug.WriteLine($"==> DropInfo: pos={InsertPosition}, item=NULL, idx={InsertIndex}");
				}
			}
			else
			{
		...
		/// <summary>
		/// Gets the item index in an ItemsControl that is the current drop target.
		/// </summary>
